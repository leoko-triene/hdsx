"""Add knowledge point relations and graph snapshots."""
from alembic import op
import sqlalchemy as sa
from sqlalchemy.dialects import mysql

revision = "0011_knowledge_graph"
down_revision = "0010_question_explanation"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    if "knowledge_point_relations" not in tables:
        op.create_table(
            ...
            sa.Column("course_id", mysql.BIGINT(), sa.ForeignKey("courses.id"), nullable=False),
            sa.Column("from_kp_id", mysql.BIGINT(), sa.ForeignKey("knowledge_points.id"), nullable=False),
            sa.Column("to_kp_id", mysql.BIGINT(), sa.ForeignKey("knowledge_points.id"), nullable=False),
    ...
    if "knowledge_point_graphs" not in tables:
        op.create_table(
            ...
            sa.Column("generated_by", mysql.BIGINT(), sa.ForeignKey("users.id"), nullable=True),
            sa.Column("reviewed_by", mysql.BIGINT(), sa.ForeignKey("users.id"), nullable=True),
